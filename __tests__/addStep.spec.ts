...
import { addStep } from '../source/math/addStep'
...
describe('addStep', () => {
  ...
  testCases.forEach(([date, Step, count, expected]) => {
    it(`should resolve ${date} addStep (${count} * ${Step}ms) to ${expected}ms`, () => {
      expect(addStep(date, Step, count).getTime()).toEqual(expected)
    })
  })

  it('should just return with the same date', () => {
    expect(addStep(0).getTime()).toEqual(new Date(0).getTime())
  })
})
